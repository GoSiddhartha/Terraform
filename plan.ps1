...
process {
	...
	terragrunt plan-all -var "tenant=$($TENENT_ID)" -var "subscription_id=$($SUBSCRIPTION_ID)" -var "client_id=$($SERVICE_PRINCIPAL_ID)" -var "client_secret=$($SERVICE_PRINCIPAL_KEY)" -out=tfplan
}